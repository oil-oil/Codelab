...
export default function (ast: Token[])  {
    ...
    ast.forEach((token, index) => {
        if (token.markup === "#" && token.type === "heading_open") {
        ...
        } else if (token.markup === "##" && token.type === "heading_open" && data[data.length - 1]) {
            const children = data[data.length - 1].children;
            children.push({
                title: ast[index + 1].content,
                content: [token],
                finish:false
            ...
            if (data.length && data[data.length - 1].children.length) {
                (data[data.length - 1].children[
                    data[data.length - 1].children.length - 1
                ].content as Token[]).push(token);
            }
        }
    });
        data.map(item => {
            return item.children.map(item=>{
                item.content = renderer.render(item.content as Token[],{
